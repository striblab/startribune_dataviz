...
d3.json('./data/taxes.json', function(error, dataLoad) {
...
function mapColor(d, race, dataCompare){
   // if (race != "redistrict"){ 
    for (var i=0; i < dataCompare.length; i++){
        if (d.properties.COUNTYNAME == dataCompare[i].PLACE){
        	var color_scale = d3.scale.linear().domain([7000,15000]).range(['#F7F7F7','#252525']);
            var total = Number(dataCompare[i].AverageTotal);
...
function mapTips(d, subject, dataCompare){
   var total = 0;
    for (var i=0; i < dataCompare.length; i++){
        if (d.properties.COUNTYNAME == dataCompare[i].PLACE){
            total = Number(dataCompare[i].AverageTotal);
            var color_scale = d3.scale.linear().domain([7000,15000]).range(['#F7F7F7','#252525']);
        }
    } 
